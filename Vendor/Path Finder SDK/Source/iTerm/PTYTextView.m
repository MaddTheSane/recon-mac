	...
	{		
		startX=-1;
             ...
             [mouseDownEvent locationInWindow].y == [event locationInWindow].y)
         {
            ...
            // TODO: Modified by Sumanth Peddamatham @ bafoontecha.com
            //  I want to handle this case myself
            if (url != nil) 
            {
               NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];      
               [nc postNotificationName:@"BAFstringFromConsoleAlternate" object:url];  
               
               //               [self _openURL:url];
            }
         }
        else if (([event modifierFlags] & NSCommandKeyMask) && [[PreferencePanel sharedInstance] cmdSelection] &&
			[mouseDownEvent locationInWindow].x == [event locationInWindow].x &&
			[mouseDownEvent locationInWindow].y == [event locationInWindow].y)
        {
            //[self _openURL: [self selectedText]];
           ...
           //  I want to handle this case myself
            ...
            {
               NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];      
               [nc postNotificationName:@"BAFstringFromConsole" object:url];  
