...
//   [self populateInterfacePopUp];   
   ...
   if ([mainSplitView collapsibleSubviewCollapsed] == NO)
      [mainSplitView toggleCollapse:collapseButton];
...
//   [workspaceAdvancedContent retain];   
   ...
   // Set-up Sidebar
//   [sideBarPlaceholder setWantsLayer:YES];
   
   [sideBarAdvancedContent setFrame:[sideBarPlaceholder frame]];
...
//   [workspacePlaceholder addSubview:workspaceBasicContent];
   ...
   [self setWindowSizeForMode:0 withSetting:0];   
   
//   int minWidth = 700; int minHeight = 550;   
//   NSSize minSize = {minWidth, minHeight};
//   
//   [mainWindow setMinSize:minSize];
   
   [[workspaceSettingsScrollView documentView] scrollPoint:NSZeroPoint];   
...
//                                                 with:workspaceBasicContent];
   ...
   if ([sender selectedSegment] == 1) 
   {
      ...
      [sideBarAdvancedContent setFrame:[sideBarPlaceholder frame]];
      [sideBarPlaceholder replaceSubview:[[sideBarPlaceholder subviews] lastObject]
                                    with:sideBarAdvancedContent];
      
      [self sizeSwitch:nil];
      
      if ([mainSplitView collapsibleSubviewCollapsed] == YES)
         [mainSplitView toggleCollapse:collapseButton];
   ...
   if ([sender selectedSegment] == 2) 
   {
      ...
      [sideBarSettingsContent setFrame:[sideBarPlaceholder frame]];
      [sideBarPlaceholder replaceSubview:[[sideBarPlaceholder subviews] lastObject]
                                    with:sideBarSettingsContent];
...
{
   [workspacePlaceholder setWantsLayer:NO];   
}

- (void)setWindowSizeForMode:(int)mode withSetting:(int)setting
{
   return;
   NSLog(@"PINGU");
   ...
   if ((mode == 0) && (setting == 0))
   {
      minWidth = 625; minHeight = 525;
      maxWidth = 800; maxHeight = 525;
   }
   // Basic - Large
   if ((mode == 0) && (setting == 1))
   {
   ...
   NSSize currentSize = [mainWindow frame].size;
   NSSize newSize = minSize;
   ...
   [mainWindow setFrame:windowFrame
                display:YES
                animate:YES];   
...
- (IBAction)sizeSwitch:(id)sender
{
   [self setWindowSizeForMode:[modeSwitchBar selectedSegment] withSetting:[sizeSwitchButton state]];
}
...
//      [sessionManager queueSessionWithProfile:[[profilesTreeController selectedObjects] lastObject]
