                     ...
                     withOutputFile:(NSString*)nmapOutput
{
   ...
   [nmapArgs addObject:[nmapOutput stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
   
   // Multiple targets must be added separately, we support comma/space separated targets
   NSArray *targetCommaArray = [target componentsSeparatedByString:@","];

   for (NSString *targetCommaString in targetCommaArray)
   {
      NSArray *targetArray = [targetCommaString componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
      
      for (NSString *targetString in targetArray)
      {
         if ([targetString isEqualToString:@""] == NO)
         {
